<!-- Thanks for contributing to Fibbers! -->

## What & why

<!-- A short description of the change and the motivation. -->

## Checklist

- [ ] Edited `src/` (not `dist/`), then ran `bun run build` and committed `dist/fibbers.js`.
- [ ] `bun run check` passes (prettier + build + parse).
- [ ] Verified in Storybook (or `test/fixtures/fixture.html` / `test/fixtures/hatabs-fixture.html` for nav-pin / hide_ha_tabs behaviour).
- [ ] Updated `README.md` / `CHANGELOG.md` if user-facing.
