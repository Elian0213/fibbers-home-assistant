/* ================================================================== *
 * fibbers-remote — pure math for the device page-swipe deck: axis lock, velocity,
 * rubber-band resistance, edge detection and the commit target. No DOM, no state,
 * so it's unit-tested directly (deck-math.test.ts). The gesture engine that calls
 * it lives in deck-controller.ts.
 * ================================================================== */
...
/** 70/30 EMA velocity, px/ms. Deliberately duplicated rather than imported from
 *  touchpad-math.ts, which stays frozen — including its import graph.
...
/** Swiper-style rubber band past the first/last device (resistanceRatio 0.85).
 * @param dx — the raw horizontal drag
 * @param width — the deck width
...
/** True when a drag of this direction has nowhere to go (first/last device).
...
/** The index to land on. Commit on half a panel of travel OR a flick. */
export const FLICK_V = 0.35; // px/ms
export const FLICK_MIN = 24; // px — a flick still needs real travel

/**
 * The device index a released drag should land on: the current one unless the drag
 * crossed half the deck or was a flick with real travel; never wraps.
 * @param sel — the current index
 * @param count — the device count
...
export function commitTarget(
  ...
  const next = sel + (dx < 0 ? 1 : -1); // drag left → next device
