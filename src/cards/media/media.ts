...
/** A member of a speaker `group:` list — an entity id, or `{ entity, name? }`. */
interface GroupMember {
  entity: string;
  name?: string;
}

/** A `favourites:` entry — a play_media target with an optional tile look. */
interface Favourite {
  name?: string;
...
/** YAML/editor config accepted by `fibbers-media`. */
export interface MediaConfig extends LovelaceCardConfig {
  entity: string;
  name?: string;
  compact?: boolean;
...
export class FibbersMedia extends LitElement implements LovelaceCard {
  ...
  setConfig(config: MediaConfig): void {
    ...
    this._seekInput = debounce((v: number) => this._seek(v), 150);
  ...
  updated(): void {
    ...
    const st = this._st();
    const upd = (st && st.attributes.media_position_updated_at) || null;
  ...
  private _supports(bit: number): boolean {
    const st = this._st();
    const f = (st && st.attributes.supported_features) || 0;
    // eslint-disable-next-line no-bitwise -- feature-bit test against a bitmask
    return (f & bit) === bit;
  }
  ...
  private _pos(): Position | null {
    ...
    if (this._playing() && a.media_position_updated_at) {
      ...
      if (!Number.isNaN(upd)) pos = base + (Date.now() - upd) / 1000;
    }
  ...
  private _allSources(): ChipItem[] {
    ...
    return cfg.sources === "auto"
      ? ((a.source_list || []) as string[]).map((s) => ({ name: s, source: s }))
      : cfg.sources.map((s) =>
  ...
  private _seek(seconds: number): void {
    ...
    const st = this._st();
    this._seekAt = (st && st.attributes.media_position_updated_at) || null;
  ...
  private _do(service: string, data?: Record<string, unknown>): void {
    this._svc(service, data).catch(() => {});
  }

  private _join(entityId: string): void {
    if (!this.hass) return;
    this.hass
  ...
  private _unjoin(entityId: string): void {
    if (!this.hass) return;
    this.hass
  ...
  private _seekFromX(e: PointerEvent): number {
    const dur = (this._pos() || {}).dur || 0;
    return (pctFromX(e.clientX, e.currentTarget as Element) / 100) * dur;
  }

  private _transportBtn(
    icon: string,
    service: string,
    opts: { big?: boolean; accent?: boolean } = {},
  ): TemplateResult {
