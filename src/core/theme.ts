...
const state: { mode: ThemeMode; mql: MediaQueryList | null } = {
  mode: "none",
  mql: null,
};

const isGlobal = (m: ThemeMode): boolean =>
  m === "fibbers-global" || m === "fibbers-global-light";

// Which palette the current mode resolves to (null = inject nothing).
function palette(): ThemeVars | null {
...
const unreachable = (k: string): boolean =>
...
function cssFor(vars: ThemeVars): string {
 ...
 */
export function applyTheme(mode: ThemeMode | string | undefined): void {
  const KNOWN: ThemeMode[] = [
    "fibbers",
    "fibbers-light",
    "auto",
    "fibbers-global",
  ...
  const normalized: ThemeMode = KNOWN.includes(mode as ThemeMode)
    ? (mode as ThemeMode)
    : "none";
