...
/** A map of HA theme-var names to their CSS values. */
 ...
 * @param vars — HA theme-var map (defaults to the dark palette)
 */
export function setGlobalVars(vars: ThemeVars = DARK_VARS): void {
  if (
    (window as unknown as { FIBBERS_DISABLE_GLOBAL_CSS?: boolean })
      .FIBBERS_DISABLE_GLOBAL_CSS
  )
    return;
...
export function clearGlobalVars(): void {
