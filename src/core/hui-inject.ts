 ...
 * ================================================================== */
import { deepFind } from "@shared/util";

/** A subscriber's CSS producer — returns the CSS to inject, or "" / null to remove it. */
export type ComputeCss = () => string | null;
 ...
 * @returns the hui-root element, or null when it isn't mounted
 */
export function findHuiRoot(): Element | null {
  return deepFind("hui-root");
}
const findPanel = (): Element | null => deepFind("partial-panel-resolver");

const subs = new Map<string, ComputeCss>(); // id -> computeCss()
const state: {
...
function paintOne(root: Element, id: string, css: string | null): void {
...
function retargetIfFallback(): void {
 ...
 * @param computeCss — () => string | ""
 */
export function injectStyle(id: string, computeCss: ComputeCss): void {
 ...
 * @param id — the <style> id passed to injectStyle
 */
