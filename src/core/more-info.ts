...
/** A child card config the modal renders for an entity. */
...
const DOMAIN_CARDS: Record<string, (id: string) => CardConfig[]> = {
...
const sensorCards = (id: string): CardConfig[] => [
...
/** <home-assistant> exposes the live hass object as an instance property. */
type HomeAssistantEl = Element & { hass?: HomeAssistant };

const currentHass = (): HomeAssistant | null => {
  if (nav.hassRef) return nav.hassRef;
  const el = document.querySelector<HomeAssistantEl>("home-assistant");
  return (el && el.hass) || null;
};

// Decide the modal content for an entity, or null to let HA handle it.
function cardsFor(hass: HomeAssistant, id: string): CardConfig[] | null {
...
/** The `hass-more-info` event detail HA dispatches (plus Fibbers' room-group extras). */
interface MoreInfoDetail {
  entityId?: string;
  siblings?: string[];
  groupName?: string;
}
...
function handle(e: Event): void {
  const detail: MoreInfoDetail =
    (e as CustomEvent<MoreInfoDetail>).detail || {};
...
const state: { on: boolean; fn: ((e: Event) => void) | null } = {
  on: false,
  fn: null,
};
 ...
 */
export function enableMoreInfo(): void {
  if (state.on) return;
  state.on = true;
  state.fn = handle;
 ...
 */
export function disableMoreInfo(): void {
  if (!state.on) return;
  state.on = false;
  if (state.fn) window.removeEventListener("hass-more-info", state.fn, true);
  state.fn = null;
  closeModal();
  teardownIfIdle();
}
