...
/** A single icon definition: a viewBox and the inner SVG markup. */
interface IconDef {
  vb: string;
  body: string;
}

/** A map of `solar:` icon names to their SVG definitions. */
...
const warned = new Set<string>(); // one console.warn per unmapped name, not per render
...
let fullPromise: Promise<IconMap | null> | null = null;
...
function iconsUrl(): string {
  return (
    (window as Window & { FIBBERS_ICONS_URL?: string }).FIBBERS_ICONS_URL ||
...
function loadFull(): Promise<IconMap | null> {
 ...
 * @param name
 * @returns svg markup, or null
 */
function iconSvg(name: string): string | null {
  const ic = name && ((ICONS as IconMap)[name] || (FULL && FULL[name]));
  if (!ic) return null;
...
class FibIcon extends HTMLElement {
  ...
  private _render(): void {
    ...
    if (!ha || ha.localName !== "ha-icon") {
      ...
      (ha as HTMLElement).style.setProperty("--mdc-icon-size", "inherit");
      this.appendChild(ha);
    }
